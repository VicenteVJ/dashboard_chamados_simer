...
const normalizeTicket = (row) => {
  ...
  const abertoEm = findValue(row, [
    'aberto em', 'abertoem', 'data abertura', 'data de abertura', 'criado em', 'criado'
  ]);
  const departamento = findValue(row, ['departamento', 'depto', 'setor', 'area', 'área']);
  const solicitante = findValue(row, [
    'usuario solicitante', 'usuário solicitante', 'solicitante', 'requester'
  ]);
  ...
  const ultimaAcao = findValue(row, [
    'data da última ação', 'data da ultima acao', 'última ação', 'ultima acao',
    'ultimaacao', 'atualizado em', 'updated'
  ]);
  ...
  let priorizado = findValue(row, [
    'ticket priorizado', 'priorizado', 'prioritario', 'prioritário', 'priority'
  ]);
  ...
  priorizado = /sim/i.test(priorizado)
    ...
    : /nao|não/i.test(priorizado)
      ? 'Nao'
      : priorizado ? priorizado : 'Nao';

  return {
    numero: numeroFix, abertoEm, departamento, solicitante, cliente,
    servico, assunto, responsavel, categoria, ultimaAcao, status, priorizado, _raw: row
  };
};
...
const normalizeOcorrenciaRow = (row) => {
  ...
  const responsavel = findValue(row, [
    'responsavel', 'responsável', 'responsavel ti', 'owner', 'atribuido', 'atribuído'
  ]);
  ...
  const titulo = findValue(row, [
    'titulo / descricao', 'título / descrição', 'titulo', 'título', 'descricao', 'descrição'
  ]);
  ...
  const validacao = findValue(row, [
    'validacao ti', 'validação ti', 'observacao ti', 'observação ti', 'validacao', 'validação'
  ]);
  ...
  return {
    ticket: ticketFix, departamento, responsavel, tipo, dataAbert,
    titulo, aliare, prioridade, impacto, validacao, statusAtual, tipoOc, _raw: row
  };
};

const parseWorkbookToRows = async (buffer, filename) => {
...
module.exports = { normalizeTicket, normalizeOcorrenciaRow, parseWorkbookToRows };
